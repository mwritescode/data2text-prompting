        ...
        if position_bias is None:
            ...
            if prefix_key_value is not None:
                position_bias = torch.cat([
                    torch.zeros((1, self.n_heads, int(seq_length), key_states.shape[2] - key_length)).to(position_bias.device),
                    position_bias
                ], dim=3) 
...
class T5Block(nn.Module):
    ...
    def forward(
        ...
        output_attentions=False,
        return_dict=True,
        prefixes_list=None # [encoder_prefix, decoder_prefix, cross_prefix]
    ):
        encoder_prefix, decoder_prefix, cross_prefix = prefixes_list
        ...
        self_attention_outputs = self.layer[0](
            ...
            use_cache=use_cache,
            output_attentions=output_attentions,
            prefix_key_value=decoder_prefix if self.is_decoder else encoder_prefix
        )
...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            encoder_prefix = prefix_key_values[i]['encoder'] if prefix_key_values is not None else None 
            decoder_prefix = prefix_key_values[i]['decoder'] if prefix_key_values is not None else None
            cross_prefix = prefix_key_values[i]['cross'] if prefix_key_values is not None else None
            prefixes_list = [encoder_prefix, decoder_prefix, cross_prefix]
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        return tuple(module(*inputs, use_cache, output_attentions, prefixes_list))
