...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        prefix_key_value: Optional[torch.Tensor] = None
...
class BartEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        prefix_key_value: Optional[Dict[str, torch.Tensor]] = None
        ...
        hidden_states, attn_weights, _ = self.self_attn(
            ...
            prefix_key_value=prefix_key_value['encoder'] if prefix_key_value is not None else None
        )
...
class BartDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            ...
            layer_head_mask=layer_head_mask,
            output_attentions=output_attentions,
            prefix_key_value=prefix_key_value['decoder'] if prefix_key_value is not None else None
        )
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        hidden_states = residual + hidden_states
        hidden_states = self.self_attn_layer_norm(hidden_states)
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_states, cross_attn_weights, cross_attn_present_key_value = self.encoder_attn(
                ...
                output_attentions=output_attentions,
                prefix_key_value=prefix_key_value['cross'] if prefix_key_value is not None else None
            )
            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
            hidden_states = residual + hidden_states
...
class BartEncoder(BartPretrainedModel):
    ...
    def forward(
        ...
        prefix_key_values: Optional[List[Dict[str, torch.Tensor]]] = None,
        ...
        if prefix_key_values is not None and attention_mask is not None:
                ...
                bsz_prefix, _, prefix_seq_length, _ = prefix_key_values[0]['encoder'][0].size()
                bsz = bsz_input if bsz_input == bsz_prefix else AssertionError("In Encoder, the bsz of input and prefix must be the same")
                attention_mask = torch.cat([
                    torch.zeros(bsz, 1, tgt_seq_len, prefix_seq_length).to(attention_mask.device), 
                ...
                if self.gradient_checkpointing and self.training:
                    ...
                    def create_custom_forward(module):
                        def custom_forward(*inputs):
                            return module(*inputs, output_attentions, prefix_key_values[idx] if prefix_key_values is not None else None)
                    ...
                    layer_outputs = encoder_layer(
                        ...
                        output_attentions=output_attentions,
                        prefix_key_value=prefix_key_values[idx] if prefix_key_values is not None else None
                    )
...
class BartDecoder(BartPretrainedModel):
    ...
    def forward(
        ...
        if prefix_key_values is not None and attention_mask is not None:
            ...
            bsz_input, _, tgt_seq_len, _ = attention_mask.size()
            bsz_prefix, _, prefix_seq_length, _ = prefix_key_values[0]['decoder'][0].size()
            bsz = bsz_input if bsz_input==bsz_prefix else None
            attention_mask = torch.cat([
                torch.zeros(bsz, 1, tgt_seq_len, prefix_seq_length).to(attention_mask.device), 
                attention_mask], dim=3)
        
        if prefix_key_values is not None and encoder_attention_mask is not None:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        ...
                        return module(*inputs, output_attentions, use_cache, prefix_key_values[idx] if prefix_key_values is not None else None)
